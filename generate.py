...
def style_mixing(generator, step, mean_style, n_source, n_target):
    ...
    images = [jt.ones((1, 3, shape, shape)) * 255]
...
def style_weight_mixing(generator, step, mean_style, n_source, n_target):
    source_code = jt.randn(n_source, 512)
    target_code = jt.randn(n_target, 512)
    source_image = generator(
    ...
    for j in range(1, 40):
        ...
        images = jt.concat(images, 0)
        jt.save_image(
            images, f'./generateSample/sample_weight_mixing/sample_weight_mixing_{j}.png', nrow=args.n_col + 1, normalize=True, range=(-1, 1)
        )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path', type=str, default='./checkpoint/020000.model', help='path to checkpoint file')
    ...
    jt.save_image(img, './generateSample/sample.png', nrow=args.n_col, normalize=True, range=(-1, 1))

    style_weight_mixing(generator, step, mean_style, args.n_col, args.n_row)
    for j in range(40):
        img = style_mixing(generator, step, mean_style, args.n_col, args.n_row)
        jt.save_image(
            img, f'./generateSample/sample_mixing/sample_mixing_{j}.png', nrow=args.n_col + 1, normalize=True, range=(-1, 1)
        )
