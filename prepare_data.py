...
def prepare(transaction, dataset, n_worker, sizes=(8, 16, 32, 64, 128)):
    ...
    with multiprocessing.Pool(n_worker) as pool:
        for i, imgs in tqdm(pool.imap_unordered(resize_fn, files)):  # problem
...
if __name__ == '__main__':
    ...
    with lmdb.open(args.out, map_size=100000000, readahead=False) as env:
